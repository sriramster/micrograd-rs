...
struct GraphNode {
    ...
    grad: f64,
    label: String,
    prev: Vec<Rc<RefCell<GraphNode>>>,
    op: Option<String>,
    backward: Option<Rc<dyn Fn()>>,
}

#[derive(Debug, Clone)]
struct Value(Rc<RefCell<GraphNode>>);
...
impl GraphNode {
    fn fmt_indented(&self, f: &mut fmt::Formatter<'_>, indent: usize) -> fmt::Result {
        ...
        for parent_rc in &self.prev {
            parent_rc.borrow().fmt_indented(f, indent + 2)?;
        }
        ...
        out.borrow_mut().backward = Some(Rc::new(move || {
            if let Some(out_rc) = weak_out.upgrade() {
                ...
                if let Some(a_rc) = weak_a.upgrade() {
                    a_rc.borrow_mut().grad += (1.0 - out_val.powi(2)) * out_grad;
                }
            }
        }));
        out
